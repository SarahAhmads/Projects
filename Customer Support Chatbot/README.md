...
### 0. Extract the Processed Data (First Time Only)
The `data/processed/` folder is stored as a ZIP file to keep the repository lightweight. **You must extract it before running the project.**

**Windows (PowerShell):**
```powershell
Expand-Archive -Path "data\processed.zip" -DestinationPath "data\" -Force
```
...
After extraction, confirm the folder `data/processed/` exists.
